...
public:
	...
	// dx,dy: tensors/edge weights in x and y direction. d_c: confidence,
	// d_b rhs, eg. u_hat; d_i: initial solution, eg. d_b or from other hirarchy level.
	// d_out: output.
	// ic: channels(# of rhs/channels of b/solution), iw: width ih: height of inputs
	...
	static int forward(float *d_x, float *d_y, float *d_c, float *d_b, float *d_i, float *d_out,
...
private:
	// no copies!
	FistaTGVHuberB2_id(FistaTGVHuberB2_id const&);
	void operator=(FistaTGVHuberB2_id const&);

	static std::vector<idStructBs> id;
};

std::vector <idStructBs> FistaTGVHuberB2_id::id = std::vector <idStructBs>(__uniqueIds__);
#undef __uniqueIds__