...
std::vector<at::Tensor> tgvInpaint_dummy( )
{
	std::cout << "Input: not present just dummy:\n";
	return std::vector<at::Tensor> {};
}

void tgvInpaint_dummy_only( )
{
	std::cout << "Dummy call:\n";
	return;
}
...
std::vector<at::Tensor> tgvInpaint_forward(at::Tensor d_x, at::Tensor d_y,
	at::Tensor d_c, at::Tensor d_b, at::Tensor d_i, int its, int id)
{
	...
	const int channelI   = d_i.size(1);
	const int channelX   = d_x.size(1);
	const int channelY   = d_y.size(1);

	if (channelX != channelY || channelX != 2)
	{
			std::cout << "Cuda Function call tgvInpaint::forward failed.\n\
			 The input to the function should have 2 channels for the diffusion tensor.\n \
			 First channel operates on gradients, second on the auxilliary variables.\n \
			 To learn a single parameter here, just expand a single variable into the 2nd channel.\n"; return {};
	}
	...
	{
			std::cout << "Cuda Function call tgvInpaint::forward failed\n\
			 The function can only handle input with 3 channels:(primary and 2 x auxilliary variables) \n"; return {};
	}
	...
	if ( d_x.type().scalarType() == at::ScalarType::Float )
	{
		...
		int result = TGVInpaint::forward( (float*) ( d_x.data<float>() ), (float*) ( d_y.data<float>() ),
...
std::vector<at::Tensor> tgvInpaint_backward( at::Tensor d_iGf, at::Tensor d_x, at::Tensor d_y,
				at::Tensor d_c, at::Tensor d_b, int its, int id)
{
	...
	if (batches != 1)
	{
			std::cout << "Cuda Function call tgvInpaint::backward failed\n\
			 The function can only handle a batchsize of one\n"; return {};
	}
	...
	if ( d_b.type().scalarType() == at::ScalarType::Float )
	{
		...
		int result = TGVInpaint::backward((float*) ( d_x.data<float>() ), (float*) ( d_y.data<float>() ),
		                                 (float*) ( d_c.data<float>() ), (float*) ( d_b.data<float>() ),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
	m.def("forward",    &tgvInpaint_forward,    "tgvInpaint forward (CUDA)");
	m.def("backward",   &tgvInpaint_backward,   "tgvInpaint backward (CUDA)");
	m.def("dummy",      &tgvInpaint_dummy,      "tgvInpaint dummy");
	m.def("dummy_only", &tgvInpaint_dummy_only, "tgvInpaint dummy2");
}
